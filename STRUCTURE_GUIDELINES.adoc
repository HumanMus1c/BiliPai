...
适用范围：`app/src/main/java/com/android/purebilibili/` 与 `design-system/src/main/java/com/android/purebilibili/`
...
- `navigation/`、`navigation3/`：当前并存的全局路由和页面注册实现。
- `design-system/`：跨页面共享的主题、组件、图标、动效和纯视觉策略，不承载业务状态或网络逻辑。
...
3. 已下沉到 `design-system`、`settings-core`、`network-core` 或 `plugin-sdk` 的公共能力继续保持单向依赖；新增拆分应渐进进行，避免一次性大重构。
