...
3. 在组件档案填写用途、禁用场景、结构、变体、尺寸、Token、状态、交互、文案、无障碍、响应式、双预设映射、Compose 入口、差距和验收。
4. 公共组件进入 `design-system`；领域专用组件留在 feature 包并明确边界。
5. 新入口应有策略或结构测试；旧入口迁移前不得宣称“唯一入口已完成”。

### 新增 Token

Token 提案必须回答：现有 Token 为什么不能表达、至少哪些场景共享、双预设如何映射、旧硬编码怎样迁移、如何验证。单一页面的一次数值通常保持领域常量，而不是全局 Token。
...
草案阶段保持 `1.1.0-draft`，首次人工评审完成后改为 `1.0.0`。任何规则改变都要更新 `CHANGELOG.md`；只更新“最后核对日期”不算版本变更。
