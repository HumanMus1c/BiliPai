...
“Miuix 主导”指默认界面的视觉气质、控件选择和完整测试以 Miuix 为准，并不表示删除 iOS 与 Material 3。三种风格像同一间房的三套装修：家具样式可以不同，但门的位置、房间用途和逃生通道不能改变。
...
3. 手机、平板和宽屏保持同一信息顺序。
4. Miuix 具有完整一致的外观，iOS 与 Material 3 保持功能兼容。
5. 动效和玻璃效果服务于层级反馈，不能牺牲可读性与性能。

## Why：为什么这样选择

- 项目已经接入 Miuix 0.9.3，并已有主题桥接、Preference、导航栏、侧栏和播放器设置等能力。
- 项目仍保存三种 `UiStyle`，直接删除兼容模式会破坏用户已有设置与使用习惯。
...
- **必须**保留 iOS、Material 3 的功能、信息、状态、导航和无障碍语义。
- **必须**从 `App*` 公共入口消费风格差异；业务页面不得直接读取 Miuix 主题对象。
- **应该**先定义语义组件，再决定三风格分别由哪个底层组件渲染。
- **可以**让不同风格使用各自适合的圆角、字重、反馈和动效。
- **禁止**为了视觉一致而伪装平台控件，也禁止因风格不同隐藏某项功能。

## 正确与错误示例

正确：三种风格的“关注”都显示已关注/未关注状态，Miuix 使用 Miuix 按钮，Material 3 使用 Material 按钮。  
错误：Miuix 有取消关注入口，而 iOS 风格只能关注不能取消。
...
- 风格设置：应用侧 `UiStyle.IOS`、`UiStyle.MATERIAL3`、`UiStyle.MIUIX`
...
选首页、搜索、视频详情/播放器、个人页、设置和通用弹层，切换三种风格。每项功能入口、状态文案、返回结果和可访问名称必须一致；只允许外观和反馈方式发生本节允许的变化。
