...
| 双预设映射 | 输入壳、焦点与清空图标可变；查询、建议和提交语义一致 |
...
| 双预设映射 | 使用各自 Switch；值、启用条件和保存结果一致 |
...
| 双预设映射 | Thumb/track 可变；范围、步长、值与禁用条件一致 |
...
| 双预设 | Preference 原生表现可不同 | 分组容器和间距可不同 |
