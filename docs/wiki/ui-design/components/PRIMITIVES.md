...
| 尺寸 / Token | 视觉高度由原生规格或圆角驱动的主题几何策略确定；触摸区独立满足无障碍下限；形状使用语义组件映射 |
...
| 结构 / 变体 | 主题化视觉容器 + 独立触控区域 + 图标 + tooltip/描述；filled 变体只用于强调状态 |
| 尺寸 / Token | 视觉容器按圆角和原生规格解析，不写死 48dp；命中区域独立满足无障碍下限，图标通常 20/24dp |
| 状态 / 交互 | normal、pressed、selected、disabled；长按 tooltip 不能成为唯一说明 |
| 文案 / 无障碍 | `contentDescription` 写动作，如已收藏时写“取消收藏” |
| 响应式 | 数量多时进入菜单，不把顶栏标题挤没 |
| 双预设映射 | 按压与容器形状可变，图标语义和命中范围一致 |
...
| 双预设 | 线条可有细微差异 | Miuix/Material 指示器可不同 |
...
为每个原语检查双主题、明暗主题、enabled/disabled/pressed/loading、360dp 长文案、1.3 倍字体、视觉/触控几何分离和读屏名称。
