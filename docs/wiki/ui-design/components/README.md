...
flowchart TD
    ...
    Domain -->|否| Proposal["提交公共组件提案与三风格映射"]
...
每个组件必须说明：用途、禁用场景、组成结构、变体、尺寸、Token、状态、交互、文案、无障碍、响应式、三风格映射、Compose 入口、当前差距和验收。
...
新组件评审时先在本目录找到唯一入口，再按对应档案检查所有状态、三风格、48dp 触摸区、字体放大和宽度变化。找不到入口时，先补文档提案而不是复制相近组件。
