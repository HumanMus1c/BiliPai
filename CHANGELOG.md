...
## v0.2.3-beta.22 (2026-09-01)

更新日志(ChangeLog)

[更改] 版本号 0.2.3-beta.22，versionCode 338
[更改] 基准为 beta.21 版本整理提交（57f6b89da，versionCode 337）

### 播放器与方向
[修复] 全屏方向按实际检测结果保留左右横屏，避免向右旋转后自动回跳
[修复] 自动连播后同步新视频身份并恢复播放画面，避免画面不可见但手势仍响应
[修复] 播放结束后仍可折叠播放器，折叠状态点击“立即播放”时使用从小到大的恢复动画
[更改] 亮度与音量手势提示切换为原生 Miuix 等级滑条

### Miuix 与导航
[添加] 新增非液态玻璃 Miuix 展示模式，统一页面导航、设置、卡片与分段控件的形状和层级
[修复] 原生 Miuix 顶栏不再吞掉页面滚动，导航按钮保持可访问的最小触摸区域
[添加] 可选的 Miuix 卡片深度预测返回动画及预览幅度限制，优化手势进度和指示器跟随

### 动态与流畅度
[添加] 动态页顶部“布局 / 发布”操作区折叠开关，折叠后保留展开入口并记住当前状态
[修复] 动态增量刷新保留阅读位置，避免列表突然跳动
[优化] 非活动页面跳过导航层截取，并仅在需要时进行页面 Chrome 捕获
