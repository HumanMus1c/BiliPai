...
[修复] 修复竖屏全屏模式下点击被弹幕层拦截导致控制面板无响应、以及主线程正则计算超时引发的闪退（ANR）（感谢 @1290000 提交 PR #809）
[优化] 竖屏流播放时隔离并卸载内联播放器组件，防止双宿主竞争与多余后台轮询，降低运行功耗（感谢 @1290000 提交 PR #809）
[优化] 精细对齐视频卡片预测性返回手势与共享元素动画：平缓 3D 姿态与漂移，手势滑动过程中内容等比变小平滑收拢回原卡片，消除打断返回时的页面跳动与回弹

## 弹幕与互动

[重构] 普通、高级及互动弹幕共用播放器视口与等比缩放管线，非全屏及小窗下随视口缩小，同步缩放描边、轨道与卡片尺寸（感谢 @CoderTCY 提交 PR #808）
[优化] 调整弹幕字号与外观样式时保留暂停帧画面并就地重测，无需重新载入弹幕流；全屏与非全屏共享显示开关及区域设置（感谢 @CoderTCY 提交 PR #808）
[优化] 收紧投票卡片并约束在播放器视口内；评分弹幕规范为五星（2/4/6/8/10 分）直选，视口切换后保留操作与交互状态（感谢 @CoderTCY 提交 PR #808）

## 文本与交互

[添加] 新增全局文本原地长按划选交互：长按直接在原地拖选文字，无需唤起繁琐弹窗
[添加] 选词过程中实时浮现已选字数提示气泡与快捷悬浮工具栏（复制、全选、搜索等），保留弹窗模式供切换

## 订阅与扩展

[添加] 引入订阅流插件支持（RSS / Atom 订阅抓取与解析展示），支持单卡片全屏容器变形转场与平滑预测返回

## 界面与装扮

[添加] UP 主头像全面支持个性装扮头像框（Pendant）与大航海（舰长/提督/总督）、大会员、官方认证多维角标展示
[优化] 优化弹性滚动标签栏（Elastic Scrollable TabRow）动效与平板端设置导航布局

## 评论与社区

[修复] 修复评论详情页主动触发反诈检测后未弹出检测结果提示的问题（感谢 @CoderTCY 提交 PR #803）
[修复] 补齐横屏评论面板的反诈检测调用与参数接线

## 致谢

- 感谢 **@CoderTCY** 提交 PR #803（修复主动触发反诈检测未提示问题）与 PR #808（统一弹幕视口缩放与互动卡片紧凑化）。
- 感谢 **@1290000** 提交 PR #809（修复竖屏全屏触摸拦截、双宿主冻结与主线程正则 ANR）。
...
[添加] 私信聊天界面全新重构：支持底部悬浮液态玻璃输入栏（Floating Liquid Glass Composer），键盘呼出与收起平滑跟随
[添加] 私信消息支持解析并展示富媒体视频卡片，完整呈现封面、标题、UP 主及播放弹幕统计数据，支持点击直达详情
[优化] 私信界面接入全局壁纸渲染管线与顶部沉浸式渐变模糊（Progressive Blur），原生对话气泡自适应屏幕宽度与对齐

## 动态与交互

[优化] 优化动态页面触控与刷新交互逻辑：单次点击平滑回顶，已在顶部时单次点击触发刷新，连续双击快速回顶并刷新
[优化] 保护动态标签页在可见性与内容更新期间的状态稳定性，避免切换时页面异常跳动
[优化] 统一返回栈调度策略：重复打开同一 UP 主空间时自动复用既有页面实例并清理瞬态参数，避免层级冗余与过渡异常

## 界面与排版

[优化] 全局规范化排版与间距设计体系（Design System Tokens）：系统性重构各功能模块字体样式与间距常量，全面适配系统字体缩放
[优化] 统一个人空间、番剧、直播、消息及收藏夹等界面的 Material 3 可滚动标签栏（Scrollable TabRow）：项目宽度自适应文本内容，消除默认最小宽度限制与文字截断，同时保证规范触控热区
[优化] 优化底栏与听视频胶囊动画性能：延后动画进度读取至绘制与布局阶段，重构听视频测量排版，大幅减少过渡重组
[修复] 修复下拉刷新指示器在空闲状态下仍持续执行动画的性能问题，降低 CPU 与 GPU 占用

## 视频与详情

[优化] 视频详情页分享操作调整为直接唤起系统原生分享选择器，精简中间交互层级
[优化] 针对折叠屏外屏及紧凑横屏窗口优化视频详情页布局：在紧凑视口下合理预留评论与详情可滚动区域，自适应 UP 主信息排版，防止标题与元数据截断
[优化] 对齐视频卡片预测返回手势与共享元素动画：平缓 3D 倾角与横向漂移，使返回过程稳定呈现内容逐步变小收拢回卡片框的质感；精准对齐取消打断态（CancelRestore）渲染管线，消除取消手势时的跳动与回弹
...
[优化] 优化播放器竖屏折叠与共享元素返回过渡动画，消除异常回弹并保持标准比例平滑缩放
[添加] UP 主个人空间点赞列表支持「查看全部」与分页加载，解除条目数量限制
[优化] 个人空间二级分段控件恢复原生样式，精简播放器音量与亮度手势指示器背景层级
[优化] 优化首页卡片深浅色文本对比度与动态取色性能，降低壁纸大半径模糊渲染负载
[修复] 规范设置弹窗排版布局与屏幕宽度适配，修复切后台时壁纸调色板状态丢失的问题
...
[添加] 视频详情页上线 PiliPlus 特性：支持弹幕池弹窗、评论区内快速搜索与便捷操作浮层
[优化] 横屏评论面板接入评论搜索与反诈检测调用，统一弹窗液态玻璃浮层路由
[修复] 完善课程（PUGV）播放链路，增加对不完整 DASH 音轨格式的容错支持
[修复] 修复字幕轮询状态更新机制，优化多字幕源检索刷新

## 播放与音频

[添加] 横屏全屏播放支持展示听视频封面与快捷媒体控制项，并优化居中排布
[优化] 调优视频小横条联动动效：消除垂直运动非线性停顿，实现平滑匀称落位
[优化] 调优收缩过程中文字与按钮的淡出过渡，避免机械硬切；动效过渡期间暂停跑马灯重测以节约 CPU 算力
[修复] 修复横屏全屏下冗余的播放控制条展示，避免界面元素重叠

## 系统与性能

[优化] 建立全局 CPU 节能调度与后台分级内存修剪机制（BackgroundMemoryTrimPolicy），切后台仅修剪过期历史图片并保留可视卡片与壁纸调色板，回前台秒级无缝自愈高斯模糊与自适应光影
[优化] 完善小窗播放（PiP / MiniPlayer）与后台活动保护机制，避免过度回收导致的重连开销

## 界面与液态玻璃

[添加] 外观设置新增“居中弹窗液态玻璃”独立开关（默认关闭），底栏与指示器等核心控件默认保留液态玻璃效果
[优化] 完善单选项设置展示形态：选择“跟随选项弹出”恢复 Miuix 原生 WindowSpinnerPreference 就地弹出体验
[优化] 居中弹窗（CENTERED_DIALOG）可按开关独立接入底栏液态玻璃渲染管线，实时跟随通透度与模糊参数，呈现半透明通透质感与边沿物理折射高光
[优化] 统一全局液态玻璃弹窗与半屏抽屉界面的高斯模糊与折射质感
[修复] 优化独立弹出菜单（MENU）容器回退策略，避免跨 Window 采样异常导致的纯白或死色背景
[修复] 修复横向滚动液态底栏外壳圆角贴边裁切，优化长内容滚动显示
[修复] 修复折叠屏卡片自适应色调提取渲染稳定性
...
[添加] 完整接入课堂/课程（PUGV）播放链路，支持 DASH 音视频流解析、倍速与清晰度切换
[添加] 补齐课程分享、离线缓存操作与全屏手势控制
[修复] 修复课程封面清晰度、课程评论隔离与播放恢复状态

## 评论与互动

[添加] 评论长按菜单支持对本人评论手动触发「检测评论状态」，复用双视角反诈探测（感谢 @CoderTCY 提交 PR #800）
[修复] 修复评论翻译 gRPC 请求编码与主线程调用异常，完善错误提示（感谢 @CoderTCY 提交 PR #799）

## 播放与音频

[优化] 确保小窗悬浮播放优先于听视频底部胶囊条展示，避免层级冲突
[优化] 修复听视频封面动画与播放无缝交接，优化返回视频详情体验
[修复] 修复手动横屏全屏下的旋转稳定性
[修复] 关闭自动横竖屏切换时手动进入横屏全屏不再被立即拉回竖屏（感谢 @CoderTCY 提交 PR #802）

## 界面与大屏

[修复] 修复 Material 3 侧边栏从其他标签切回首页时的点击停顿延迟，实现瞬发切换
[修复] 修复平板及横屏分栏模式下设置搜索栏无法打开的问题
[优化] 优化个人空间二级标签按内容自适应宽度，避免长标签挤压布局
[添加] 首页视频卡片支持封面自适应色彩提取与壁纸色板联动（外观设置可配）

## 致谢

- 感谢 **@CoderTCY** 提交 PR #799（修复评论翻译异常）、PR #800（支持手动触发评论反诈检测）与 PR #802（修复关闭自动横竖屏时手动全屏退出问题）。
...
[优化] 重构视频播放器跨端挂载架构，利用 Compose movableContentOf 实现手机竖屏、折叠分屏与大屏三栏模式间播放器 Surface 无缝流转，彻底解决折叠屏切换时的页面重组与黑屏重新协商问题
[添加] 支持小窗与桌面画中画（PiP）视频自适应长宽比例及双指无级缩放手势
[修复] 修复折叠屏封面全屏二次进入、横竖屏旋转循环及画中画空安全调用异常

## 音乐与音频

[添加] 音频播放器上线 3D CD 唱片架（Cover Flow）与底部悬浮胶囊控制条，适配单曲与播放队列视图
[添加] 针对折叠屏 Tabletop 悬停模式适配 3D 唱片架上下分屏布局，消除残留黑边
[优化] 调优歌词候选匹配算法，增强磨砂玻璃扩散光晕质感，优化歌词与顶部控制项间距
[修复] 移除听视频封面跟随播放/暂停状态缩放的动画效果，固定保持 1.0x 原始比例呈现

## 评论与互动

[添加] 引入 gRPC TranslateReply 翻译接口，评论详情与二级回复支持一键翻译与原文对照（感谢 @CoderTCY 提交 PR #795）
[修复] 修复评论数据反序列化过程中丢失的 dialog 关联字段与回复类型映射

## 界面与动效

[修复] 纠正 AppUiScalePreset 界面缩放档位 densityMultiplier 方向错误，统一缩放语义（感谢 @CoderTCY 提交 PR #796）
[优化] 完善 Miuix 主题关闭液态玻璃模式下的全局分段选择器与顶栏胶囊高亮显示，增强深色模式对比度
[优化] 支持顶部 5 阶非线性纯色渐进消隐图层与 Miuix 渐进模糊独立共存，丰富沉浸式顶栏配置
[优化] 优化返回桌面/前页动效，引入背景 4% 黄金缩放与漫反射下沉光影

## 动态、空间与基础

[优化] 规范动态卡片标题展示逻辑，置顶标题并支持专栏文章、转发动态及失效占位
[优化] 个人空间支持展示 UP 主 IP 属地与身份认证标签，规范平板端背景横幅高度
[优化] 账号会话与快照数据读取全面脱离主线程与 Compose 组合线程，消除卡顿隐患

## 致谢

- 感谢 **@CoderTCY** 提交 PR #795（移植评论翻译功能）与 PR #796（纠正界面缩放档位配置）。
...
[添加] 首页、个人空间及搜索页支持双指捏合手势平滑切换网格列数，支持跟手弹性缩放动效与自适应 HUD 胶囊提示，外观设置中提供独立开关
[添加] 悬浮回顶按钮支持长按自由拖拽与停靠位置记忆，并扩展至个人主页与相关视频推荐流
[添加] 首页轮播图新增触摸跟手视差、边缘微阴影与按压回弹效果
[优化] 优化极速平滑回顶体验，防止拖拽松手时位置回弹

## 动效与性能

[优化] 优化视频卡片展开与预测性返回过渡动效，消除过渡黑卡与二次弹跳
[优化] 优化首页信息流滚动管线，消除冗余重组，提升滑动流畅度
[优化] 优化全局悬浮胶囊指示器与液态玻璃渲染性能，改善拖拽跟手度

## 个人空间与大屏

[优化] 对齐个人主页头部横幅、关注与粉丝数据展示及详情交互，统一关注状态逻辑
[修复] 修复个人空间私信与关注按钮宽度挤压遮挡的问题
[优化] 优化折叠屏与平板大屏视频布局排版与弹幕输入栏自适应交互

## 通知与设置

[添加] 消息通知范围拆分为私信、回复、@我、点赞、系统通知五个独立子开关，支持精细化订阅偏好设置（感谢 @CoderTCY 提交 PR #789）
[优化] 优化旧版消息中心通知偏好的一次性迁移与清理机制（PR #791）
[优化] 优化通知范围图标与语义匹配，区分私信、回复、点赞、系统通知等场景

## 致谢

- 感谢 **@CoderTCY** 提交 PR #789、PR #791，贡献了消息通知中心独立分类开关与配置迁移清理能力。
...
[添加] 消息中心评论通知支持跳转至独立“评论详情”页，准确定位对应楼层与回复，并支持直接回复、点赞、点踩与一键前往源视频/动态
[添加] 后台消息通知的“消息中心”总开关拆分为私信、回复我的、@我、收到的赞、系统通知五个独立子开关，并保留旧总开关关闭后的静默偏好

[修复] 修复消息中心评论跳转及深层路由未注册导致的闪退问题

## 首页与导航

[添加] 设置中支持完全隐藏首页顶部标签页，隐藏后仍完整保留左右滑动手势切换页面

[添加] 设置中新增“仅折叠搜索栏”模式，并同步适配首页、搜索页与收藏页的滚动隐藏

[优化] 优化双击底栏/状态栏回顶逻辑，平滑回顶并同步恢复顶部栏显示

[优化] 底部悬浮底栏在关闭液态玻璃时支持单独开启背景模糊与微投影，提升复杂背景可读性

[调整] 优化顶部标签下划线贴合效果，修复 Miuix 分段控件文本截断与选中项背景问题

## 视频与动效

[添加] 底部迷你播放条新增共享元素过渡与弹性下落触底动画，提升收起与展开流畅度

[优化] 优化竖屏视频进入时的封面过渡与黑屏播放处理，消除过渡闪屏现象

## 大屏与折叠适配

[添加] 平板与折叠屏大屏双栏布局适配 AI 视频总结与视频笔记展示

[优化] 优化大屏双栏布局与相关推荐流，移除多余分栏把手，消除重复推荐项

[优化] 优化平板弹幕控制层状态流，提升大屏连续播放流畅度

## 动态与空间

[优化] 优化动态页侧边 UP 主列表的滚动跟手度与头像预加载体验

[修复] 修复个人空间顶部信息在部分机型上被系统状态栏与顶栏遮挡的问题

[优化] 优化个人主页顶栏毛玻璃与沉浸式滚动效果，规范磨砂遮罩范围
...
[测试] 测试后台消息通知功能，验证私信、互动消息、系统消息、关注更新与开播提醒的通知触发、去重及后台检查行为

---

## 后台消息通知

[添加] 设置 → 隐私与权限新增“消息通知”，支持私信、互动及系统消息、关注 UP 更新和开播提醒；总开关默认关闭，通知范围可分别设置

[添加] 提供“省电优先”和“更及时”两档检查频率，可选常驻后台，并可从常驻通知中停止服务；周期检查始终保留作为兜底

[更改] 新开启检查时先建立静默基线，按账号保存去重状态；关闭总开关后清除通知与基线，重新开启时不补发关闭期间的消息

[说明] 需要登录并授予通知权限；系统周期调度可能延迟，常驻模式会增加耗电，也仍可能被系统终止
...
[更改] 统一折叠屏显示上下文、窗口自适应信息与播放器全屏状态，改善 Book、Tabletop 和自然横屏姿态下的布局切换与播放器封面衔接

[添加] 视频详情页支持大屏双栏布局：左侧播放器与视频信息，右侧评论、相关推荐、合集和 UP 投稿；按窗口尺寸调整播放器、侧栏和标签宽度

[添加] 平板右侧内容栏支持原生收起/展开把手；左右分栏均可单独隐藏，另一侧平滑扩展，按钮位于中部边界并适配 Miuix/Material3

[修复] 调整大屏分栏把手的触摸层级，确保收起与展开按钮在双栏内容上方可正常点击

[修复] 修正接近平方的折叠屏布局未接入侧栏收起状态导致把手点击无效的问题

[调整] 收窄小横条与底部导航栏之间的垂直间距，保留少量视觉缝隙

[修复] 为接近平方折叠布局补齐左侧收缩开关，并延后大屏把手显示到详情入场过渡完成

[调整] 详情页骨架屏在入场过渡期间保持动画，避免首次进入时显示静态骨架

[调整] 将大屏分栏收缩开关改为上下排列，分离左右操作的触摸区域，避免两个方向误触成同一动作

[修复] 移除平板左侧重复简介区域，修正大屏详情页重复相关推荐、单评论标签拉伸、合集列宽和顶部标签显示问题

[更改] 空间、首页及视频详情的 Miuix 非液态玻璃控件改用原生选中态、图标文字组合和按内容宽度布局，改善平板与宽屏显示

[修复] Miuix 主题下动态页顶部分类按钮按内容宽度显示，避免单个“全部”标签被父容器撑满

[修复] 动态页顶部操作按钮固定贴合右侧，避免紧凑分类按钮改变其位置

[修复] 动态页顶部分类与布局菜单按剩余宽度自适应，恢复分类点按切换并确保左右/顶部布局入口可见

[修复] 视频详情页在渐进模糊与 Miuix 主题下收紧“最热/最新”排序控件宽度，避免铺满评论顶栏

[调整] 全局移除 Miuix 非液态玻璃标签项背后的多余 dock 轨道，仅保留标签自身的选中态与点击区域

[修复] 番剧/影视双标签同步移除 Miuix 分支的外层 dock，避免仍显示整条中间背景

## 其他相关调整

[修复] 平板播放器方向策略、共享全屏状态和折叠屏返回交接，避免方向切换与窗口姿态变化时的状态错乱

[更改] 动态评论排序、首页分类、空间二级分类和视频详情标签恢复紧凑尺寸，减少宽屏下的无意义拉伸与文字截断

本版本整理范围：beta.36（`150880c`）至当前主线，重点覆盖平板、宽屏与折叠屏适配；未运行编译或实机验收。

v0.2.3-beta.37(8b5a2fe)
...
正式包：`BiliPai-0.2.3-beta.36.apk`
Dev 包：`BiliPai-0.2.3-beta.36-dev.apk`

## 直播

[添加] 醒目留言浮层新增独立的弹幕显示开关，关闭弹幕后可保持浮层隐藏 @cunminF

## 设置与默认配置

[添加] 设置分享页支持保存带自定义名称的本机配置，并可点击配置一键恢复

[更改] 新用户首次启动按内置配置初始化；首页横幅和视频转场实时模糊默认关闭

## 页面与主题

[修复] 修复首页、搜索、空间、列表等页面切换时顶部主题色短暂闪变和液态玻璃状态断层

[修复] 修复平板与宽屏下空间、视频卡片、导航页面的部分尺寸和切换适配问题

## 视频详情与返回

[更改] 视频小横条进入详情页时保留来源快照，用于普通返回过程中的视觉交接

[更改] 暂不为视频小横条来源接入预测性返回手势

v0.2.3-beta.36(150880c)
...
正式包：`BiliPai-0.2.3-beta.34.apk`
Dev 包：`BiliPai-0.2.3-beta.34-dev.apk`

## 底栏、搜索与视频小横条

[添加] 底部导航、搜索胶囊与视频小横条加入联动收拢/展开效果，随首页滚动在完整导航与紧凑状态之间自然切换

[更改] “听视频小横条”调整为“视频小横条”；进入视频详情后返回首页等页面时，即使当前视频未继续播放，也可以保留当前视频入口

[优化] 视频小横条标题过长时支持连续滚动显示，减少标题被截断的情况

[更改] 原“底栏搜索入口”调整为“底栏搜索联动”，独立搜索胶囊可与导航和视频小横条共同参与滚动联动

[更改] 移除“底栏搜索布局”和“搜索框自动展开”两组重复的细分设置，简化底栏搜索配置

[优化] 导航、视频小横条和搜索区域保持独立胶囊表面，并重新调整联动弹簧、收拢比例和搜索胶囊尺寸，使切换过程更自然

## 首页与导航

[修复] 首页关闭“显示完整视频卡片内容”时改为按整行对齐视频卡片，避免发布时间等元数据长度不同导致左右两列逐渐错位；完整内容模式继续保留瀑布流布局

[修复] 重复点击首页或触发回顶时，列表先真正返回顶部，再恢复头像、顶部栏和分类栏，避免顶部控件比内容提前一帧出现

[修复] 修复顶部栏重新展开后分类 Tab 仍保持隐藏的问题

[更改] 更新 Miuix 导航颜色与主题图标适配，首页顶部操作按钮、分类导航及底栏图标会按照当前界面风格选择对应图标体系

[优化] 推荐与历史导航图标保留描边到选中态的视觉变化；其他 Miuix 导航图标保持相同轮廓，由移动中的选中颜色表现状态，避免切换过程中图标轮廓错位和重影

[修复] 液态指示器滑动过程中图标颜色与指示器位置保持同步，并改善不同导航项之间的兼容性

## 视频详情与评论交互

[添加] 播放设置新增“详情页控件随滚动隐藏”开关，开启后在评论页向下浏览时自动收起顶部标签、发弹幕与评论排序控件，返回顶部后恢复显示

[修复] 详情页滚动隐藏仅作用于评论页面，在“简介 / 评论”横向切换过程中暂停折叠处理，避免抢占滑动手势或误隐藏控件

[修复] 评论页 Tab 横向切换时优化顶部评论控件的显示状态，减少切页过程中残留和闪现

[修复] 关闭渐进模糊后，视频详情恢复普通页面布局、顶部间距与原生评论标题，不再继续沿用沉浸式浮层布局

[修复] 评论排序栏、评论标题与渐进模糊区域进一步统一布局关系，减少悬浮控件与正文内容重叠

## 播放器与视频返回

[修复] 竖屏全屏播放不再被“播放器缩小自动暂停”逻辑误判为播放器已经收起，避免进入竖屏全屏后视频被意外暂停

[修复] 视频详情返回转场仅在卡片交接阶段保留媒体封面圆角，进入稳定的详情播放器后恢复矩形播放器区域

[修复] 平板及大屏设备的视频返回转场使用正确的播放器宿主裁剪，避免返回过程中残留视频卡片圆角

[优化] 视频卡片打开与返回转场降低动态模糊半径更新频率及手势阴影计算开销，同时保留最终模糊强度和转场效果，降低高刷新率设备上的渲染负担

## 液态玻璃与 Miuix

[更改] Miuix 更新至较新的 `0.9.4-5157b503-SNAPSHOT` 上游快照

[修复] 首页、动态、番剧、搜索、个人空间、列表、稍后再看及音乐等页面在加载态与正式内容切换过程中持续预热背景采样，减少液态玻璃短暂消失、闪黑或突然退回普通底色的问题

[修复] 搜索顶部栏及全局液态玻璃组件在页面内容切换时保持更连续的 Backdrop 状态

[修复] 关闭模糊或液态玻璃时补齐普通不透明顶部区域，并使顶部背景与当前页面底色保持一致

[修复] Miuix 设置页面背景重新与官方 Surface 基色保持一致，改善深色模式下黑色、灰色区域出现明显视觉断层的问题

[优化] 非液态玻璃状态下提高 Miuix 标签文字与背景的对比度

[优化] 紧凑型 Miuix 分段控件保持合理宽度，并去除多余的分段间隙

## 稳定性与兼容性

[修复] 修复 Release 构建经过 R8 优化后，在部分 Android 17 环境可能因字节码校验异常导致启动 `VerifyError` 的问题

[优化] 调整动态卡片内部参数组织并增加针对性的 R8 保护规则，降低代码合并和优化阶段产生异常字节码的风险

[优化] 进一步降低视频共享元素、模糊背景与预测返回动画中的重复计算，改善复杂页面转场时的性能稳定性

## 其他细节

[优化] 展开后的底栏搜索胶囊尺寸更加紧凑，与导航栏和视频小横条的视觉比例更加统一

[修复] 多处关闭液态玻璃、渐进模糊或动画后的降级界面补齐背景、间距与状态恢复逻辑，避免关闭特效后仍残留沉浸式布局

## v0.2.3-beta.33 (2026-09-11)

更新日志(ChangeLog)

[更改] 版本号 0.2.3-beta.33，versionCode 357
[更改] 正式包 BiliPai-0.2.3-beta.33.apk，Dev 包 BiliPai-0.2.3-beta.33-dev.apk
[更改] 基准为 beta.32 版本整理提交（e8c5ec7d，versionCode 354）

### 底栏与液态指示器
[修复] 底栏指示器按压与拖拽形变不再互相覆盖，点击与滑动切换更跟手
[修复] 指示器松手后立即收缩归位，不再残留拉伸变形
[更改] 底栏指示器静止高度与按压膨胀范围调整，紧凑底栏不再被放大成超大气泡
[修复] 分区侧栏选中指示器与主页底栏使用同一套按压比例与选中色

### 评论与图片回复
[添加] 评论输入框恢复图片上传入口，楼中楼回复同样支持发送图片
[修复] 不再因编辑器状态错误禁用发图，评论区图片发布恢复可用
[修复] 修复回复他人时回复目标丢失、误发成新评论的问题
[更改] 回复他人时自动带上“回复 @昵称”前缀，发送成功后关闭输入框

### 翻页与 Tab 切换
[更改] 底栏、首页分类、收藏夹分栏与直播互动面板统一为逐页过渡，远距离切换不再硬切
[修复] 长距离拖动顶部 Tab 指示器时标签栏自动跟随，指示器不再滑出可视区域
[修复] 收藏夹分栏首次进入直接定位，后续切换才播放过渡动画

### 图片选择与屏幕方向
[更改] 选择图片时优先拉起系统相册，无可用相册的设备自动回退系统取图入口
[适配] 播放器全屏时保留用户请求的屏幕方向，平板与大屏设备不再被系统忽略

### 评论排序控件
[修复] 评论排序 Dock 按压时不再带动整体位置下移
[修复] 评论排序 Dock 与标题栏重新对齐，指示器不再错误折射下方评论内容
[修复] 保留液态指示器完整的按压与拖拽膨胀空间，避免边缘被布局裁切

### 横向视频卡片与返回转场
[更改] 首页单列、历史记录、个人空间与相关推荐统一横向视频卡片的封面比例、信息间距和操作区布局
[修复] 收藏夹、稍后再看及横向视频卡片跳转时记录实际封面尺寸与覆盖层，返回页面不再出现封面跳变或错位

v0.2.3-beta.33(f9271557)

## v0.2.3-beta.32 (2026-09-10)

更新日志(ChangeLog)

[更改] 版本号 0.2.3-beta.32，versionCode 354
[更改] 正式包 BiliPai-0.2.3-beta.32.apk，Dev 包 BiliPai-0.2.3-beta.32-dev.apk
[更改] 基准为 beta.31 版本整理提交（3827549c8，versionCode 353）

### 液态指示器与 Dock 动效
[更改] 液态指示器静止高度由 48dp 调整为 56dp、按压峰值 78dp，紧凑 Dock 不再被放大成超大气泡
[更改] 指示器拖拽改为按方向同时纵向拉伸与横向压缩，滑动过程中的液态形变更明显
[更改] 加快指示器按压缩放与形变弹簧节奏，点击与滑动切换更跟手
[修复] 指示器释放后改为平滑回弹归位，修复松手瞬间瞬跳与二次回弹
[修复] 指示器拖拽速度按实际可选数量归一化，双项与三项分段控件不再被过度放大
[修复] 液态指示灯膨胀不再被 Dock 视口与裁剪父容器截断，紧凑 Dock、评论区表情面板与全局液态 Tab 保持完整扩散
[修复] 液态玻璃开关不再改变 Miuix 原生浅色底色调，外观配色在开关前后保持稳定
[修复] 分区侧栏选中指示器恢复与主页 Dock 一致的选中色与按压比例

### 平板与折叠屏适配
[适配] 窗口尺寸类与折叠姿态改用 Material 3 自适应窗口信息获取，平板与折叠屏分栏判定更准确

### 视频与直播
[修复] 相关视频返回转场的飞入封面沿用媒体圆角裁剪，交接前不再露出直角边缘
[修复] 直播播放页在平板与折叠屏点击返回不再先退出全屏，避免聊天分栏闪现

v0.2.3-beta.32(e8c5ec7d)

## v0.2.3-beta.31 (2026-09-10)

更新日志(ChangeLog)

[更改] 版本号 0.2.3-beta.31，versionCode 353
[更改] 正式包 BiliPai-0.2.3-beta.31.apk，Dev 包 BiliPai-0.2.3-beta.31-dev.apk
[更改] 基准为 beta.30 版本整理提交（be6d1c6a1，versionCode 352）

### 个人主页与空间交互
[添加] 外观设置新增个人主页“编辑资料”按钮开关，默认保持隐藏并支持在设置中心快速搜索
[优化] 彻底移除个人主页自定义壁纸与动态视频背景底部的分层压暗阴影，解决背景截断发暗问题，呈现纯净无遮挡的背景画面
[修复] 优化个人主页 Dock 栏与 Tab 切换交互，移除多余横向滚动容器，解决指示器拖动时整体 Dock 栏被压缩变形的问题；支持液态指示器拖拽放大突破 Dock 边界
[优化] 优化个人空间头像与头部区域间距，对齐骨骼屏加载位置并提升视觉稳定性

### 番剧与影视中心
[添加] 番剧影视中心新增液态玻璃一键回顶按钮，长页面浏览时一键平滑回到顶部
[修复] 彻底修复番剧页面因子组件错误采样父级 Backdrop 导致的循环自采样 RenderThread 闪退问题
[优化] 精准适配番剧影视页系统手势及底部导航栏边距（WindowInsets.navigationBars），解决回顶按钮贴底与列表底部内容被遮挡的问题

### 全局一键回顶与液态玻璃
[优化] 全面升级全站各页面的一键回顶浮钮为真实 Backdrop 液态玻璃折射效果与物理按压反馈
[优化] 统一并补全动态、视频评论区、搜索结果、分区分类、直播详情等各级页面的回顶按钮视觉与手感

### 界面规范与细节优化
[修复] 统一横向视频卡片封面圆角为 AppShapes.mediaCover 媒体规范，保持全页面圆角一致性
[优化] 优化动态顶部 Tab 栏指示器过渡动效，滑动与点击切换更顺滑跟手
[优化] 动态九宫格及转发内容图片展示样式优化，提升卡片质感
[优化] 视频合集抽屉弹窗高度与底部导航栏边距自适应优化

## v0.2.3-beta.30 (2026-09-09)

更新日志(ChangeLog)

[更改] 版本号 0.2.3-beta.30，versionCode 352
[更改] 正式包 BiliPai-0.2.3-beta.30.apk，Dev 包 BiliPai-0.2.3-beta.30-dev.apk
[更改] 基准为 beta.29 版本整理提交（70f8d8362，versionCode 349）

### 视频详情与评论交互
[修复] 统一横向视频卡片封面为全圆角媒体规范，修复空间投稿、首页单列、相关推荐及个人卡片封面右侧切为直角的问题
[修复] 修复楼中楼预测返回手势过程中“简介／评论／发弹幕”操作栏消失的问题，直接露出原视频详情页并保留原评论列表滚动位置
[优化] 楼中楼返回手势联动底层原视频详情页的 Miuix 导航模糊效果，随返回拖拽进度平滑消退与复原
[修复] 修复相关视频多层跳转返回时的导航拦截状态，并保持页面卡片过渡背景 Edge-to-Edge 铺满

### 动态与大图浏览
[添加] 图片大图预览新增 LivePhoto（实况照片）播放支持，支持长按或交互触发实况视频播放
[优化] 动态图片网格显示比例与富文本话题标签跳转体验
[优化] 调整动态详情与评论交互链路，优化评论列表返回与抽屉状态管理

### 个性化壁纸与材质表现
[添加] 个人主页与外观设置支持导入本地视频（MP4）及动图（WebP/GIF）作为动态背景壁纸
[优化] 统一主页与各级二级页面的卡片过渡模糊效果，增强全页面景深表现与背景连贯性
[优化] 修复音乐播控与番剧页面的液态玻璃采样边界，优化分段控件指示器边缘遮罩与圆角样式

### 个人空间与设置中心
[优化] 个人空间顶部头图解析逻辑，优先匹配合集头图与默认图，并调整头部操作按钮与 Tab 栏布局
[添加] 设置搜索支持历史记录本地持久化，支持单条删除与一键清空搜索记录
[优化] 提升骨骼屏呼吸对比度与节奏感，并在设置中提供柔和加载动画与搜索提示词偏好开关
[优化] 调整稍后再看页面工具栏标题显示与搜索筛选器对比度

## v0.2.3-beta.29 (2026-09-09)

更新日志(ChangeLog)

[更改] 版本号 0.2.3-beta.29，versionCode 349
[更改] 正式包 BiliPai-0.2.3-beta.29.apk，Dev 包 BiliPai-0.2.3-beta.29-dev.apk
[更改] 基准为 beta.28 版本整理提交（1e2752a70，versionCode 346）

### 视频详情与评论区交互重构
[修复] 彻底修复视频详情页评论列表下滑滚动时分段栏（简介/评论、发弹幕）意外收折隐藏的问题，保持该操作行常驻显示
[修复] 修复楼中楼评论预测性返回手势过程中，在开启下滑折叠播放器时简介分段栏组件消失的问题
[优化] 完善视频详情页多层级手势与嵌套滚动消费顺序，平滑保留下滑缩放播放器功能
[优化] 楼中楼对话抽屉与预测性返回动画精细化，支持手势滑动无缝跟手并正确恢复列表可见性

### 听视频与全局播控体验
[添加] 新增全局液态玻璃常驻播控条（Now Playing Bar），支持独立播放控制与封面转速动效
[优化] 听视频模式全面对齐原生沉浸设计，支持波浪动态进度条与手势平滑 Seek
[优化] 听视频播放器控制条与底部导航栏位移动画平滑联动，封面/歌词分段栏常驻保活
[优化] 单独提供随机/单曲循环独立开关，听视频直接开启无需额外跳转

### 界面材质与模糊自适应
[优化] 搜索结果栏、追番与个人空间支持全视口渐进模糊与液态分段控件（Liquid Segmented Control）
[优化] 隔离 LayerBackdrop 采样边界与 Dock 容器，杜绝自我采样导致的模糊递归崩溃
[优化] 统一各级页面 Miuix 连续曲率（Squircle）与深色模式指示器反色逻辑

### 致谢
[致谢] 特别鸣谢 BBPlayer 开源项目在听视频波浪进度条与播控交互设计上的灵感与参考

## v0.2.3-beta.28 (2026-09-08)

更新日志(ChangeLog)

[更改] 版本号 0.2.3-beta.28，versionCode 346
[更改] 正式包 BiliPai-0.2.3-beta.28.apk，Dev 包 BiliPai-0.2.3-beta.28-dev.apk
[更改] 基准为 beta.27 版本整理提交（c8decece6，versionCode 344）

### 顶栏材质与渐进模糊重构
[重构] 「顶部渐进模糊」与「顶部栏磨砂」彻底解耦为独立开关，支持原子化互斥关闭
[修复] 消除顶部状态栏下方的渐变断层阶梯横线（`startFraction` 调平至 0f），实现顶栏到内容的无缝羽化过渡
[修复] 修复渐进模糊开关状态反向生效的问题，确保开关状态与底层渲染精准对应
[优化] 动态、搜索、分区、稍后再看与二级设置页全面统一渐进模糊与磨砂互斥策略

### 主题规范与视觉对齐
[优化] Material 3 主题顶栏对齐官方边缘贴顶设计（Flat Edge-to-Edge），保留经典平滑下划线
[优化] MD3 顶部 Tab 下划线支持点击与滑动时的弹性拉伸与位移动效（Elastic Stretch-and-Slide）
[优化] MD3 主题二级设置页面标题居中对齐，严格遵循 Material 3 规范
[优化] MIUIX 主题一级设置页面标题左对齐于导航返回键右侧，对齐系统设计语言
[优化] MIUIX 主题非液态玻璃模式下顶部指示器与文字恢复中性色，并引入 MIUIX 专属 Spring 弹性回弹物理动效
[重构] MIUIX 分段控制组件（Segmented Control）升级为 8dp 连续平滑连续曲率（Squircle），去除外层多余 Dock 容器底板，并消除 Monet 动态取色带来的杂色污染

### 分段控制器与 Tab 交互
[修复] 彻底解决 UP 主空间、动态、分区分栏滚动时的文字截断与省略号问题，自适应长标题与平滑横向滚动
[修复] 修复 Miuix TabRow 指示器因并发滚动调用引起的偏移不同步与跳动
[优化] 视频详情页分段标签在液态玻璃开启时自动启用液态分段质感

### 竖屏直播间深度重构与沉浸交互
[添加] 竖屏直播间新增主播胶囊栏，集成一键关注、排名入口与双击点赞连击光效
[重构] 直播表情抽屉全面升级，支持表情包分栏 Tab 与 6 列高密度网格排版
[优化] 直播屏蔽词面板升级为 Input Chips 与卡片化布局，管理更高效直观
[优化] 直播醒目留言（SC）悬浮卡片与弹幕聊天滚动性能优化，大幅提升平滑度
[优化] 竖屏直播悬浮控件接入 Haze 背景实时磨砂与高质感光效

### 动态与富文本多端路由
[优化] 动态富文本链接全面支持应用内路由，点击 UP 主、话题、BVID 等直接精准跳转详情页
[优化] 动态操作按钮（转发、评论、点赞）采用中性灰容器底色与文字，避免高对比抢视线

### 播放与转场体验
[修复] 听视频页面顶部状态栏沉浸修复，渐变背景铺满状态栏全屏区域
[优化] 全局尊重用户自定义的预测性返回动画风格
[优化] 优化视频卡片转场深度透视层（Depth Layers）内存分配，降低 GC 抖动并及时释放转场驱动控制器
[修复] 退出横屏全屏时自动收起弹出的评论抽屉，避免遮挡页面

## v0.2.3-beta.27 (2026-09-07)

更新日志(ChangeLog)

[更改] 版本号 0.2.3-beta.27，versionCode 344
[更改] 正式包 BiliPai-0.2.3-beta.27.apk，Dev 包 BiliPai-0.2.3-beta.27-dev.apk
[更改] 基准为 beta.26 版本整理提交（0ba32f760，versionCode 343）

### 分段导航与液态指示器
[修复] 彻底修复分段选择栏横向滚动时的直角外溢问题，滑动指示器左右两侧均保持平滑圆角包裹
[优化] 搜索结果页视频筛选栏重构为完整显示 3 项，支持液态指示器手势直接拖拽滑动，对齐 UP 主空间操作手感
[优化] 搜索分类栏升级为自适应滑动液态轨道（Scrollable Tab Rail），支持多项平滑拖动与即时跟手
[修复] 导航指示器支持长文本防截断（`allowLabelOverflow`），彻底杜绝文字省略与挤压
[修复] 消除大屏与平板端下双层 Dock 堆叠与直角截断，完善两项紧凑 Miuix 分段样式

### 搜索与大屏平板排版
[添加] 引入搜索平板排版策略（`SearchTabletLayoutPolicy`），搜索结果全面适配平板与折叠屏多列网格
[优化] 搜索结果卡片对齐首页网格，支持 4~6 列自适应排版、最大 1280dp 限制与单列 840dp 居中约束
[优化] 尊重用户自定义平板卡片列数偏好，充分提升大屏展示效率

### 动态与富文本互动
[修复] 动态模块反序列化引入 `DynamicModulesFlexibleSerializer` 容错，稳健解析多样异构数据结构
[添加] 新增动态详情容灾降级策略（`DynamicDetailFallbackPolicy`），详情网络失败或离线时优先使用列表缓存卡片渲染
[优化] 完善动态增量刷新与分页注册策略，防止重复请求与列表阅读位置跳变
[优化] 动态富文本策略（`DynamicRichTextPolicy`）增强图文混排与高亮交互
[优化] 动态评论抽屉（`DynamicCommentSheet`）支持全屏拖拽展开与暗色模式视觉统一

### 弹幕高级过滤
[添加] 弹幕屏蔽关键词系统支持正则表达式前缀语法（`regex:`、`re:`、`r=` 与 `/.../`）
[优化] 正则表达式分词智能保留花括号量词逗号（如 `\d{1,3}`），避免规则被逗号拆分破坏
[优化] 弹幕设置面板提供规则规范化提示与去重处理

### 转场动效与预测性返回
[优化] 优化卡片预测性返回手势曲线，动态插值圆角（16dp 至 32dp 平滑过渡），消除转场撕裂
[优化] 优化多层级嵌套返回时的背景深度透视和透明度曲线，消除转场卡顿与边缘缝隙

### 系统稳定性与崩溃防护
[修复] 修复三星 Android 13 设备启动时毛玻璃特效引发的底层 Native 崩溃并增加防护隔离
[修复] 修复折叠屏外屏横屏冷启动时的布局抖动与错误触发
[修复] 完善崩溃墓碑（tombstone）与 `ApplicationExitInfo` 异常原因解析，增强系统级兼容性

比较：[beta.26 整理提交…main](https://github.com/jay3-yy/BiliPai/compare/0ba32f760...main)。本次仅更新源码与文档，未编译、打包或进行实机验收。

---

## v0.2.3-beta.26 (2026-09-06)

更新日志(ChangeLog)

[更改] 版本号 0.2.3-beta.26，versionCode 343
[更改] 正式包 BiliPai-0.2.3-beta.26.apk，Dev 包 BiliPai-0.2.3-beta.26-dev.apk
[更改] 基准为 beta.23 版本整理提交（d4852346d，versionCode 340）

### 卡片转场与手势
[修复] 视频卡片飞回时同步标题、播放量与封面信息，返回曲线与进场曲线镜像对称，避免仅封面变形
[修复] 修复点击进场时的首帧闪烁与白边，飞卡未实际覆盖前保持原位绘制，覆盖后才清空原卡槽
[修复] 飞卡封面与详情页内联播放视口精准对齐，手动封面裁切与图片保持一致
[修复] 嵌套跳转视频时同步交接状态，隐藏底部导航栏，避免内联播放尺寸挤压

### 导航与液态指示器
[更改] 全局液态胶囊（Liquid Dock）统一为即时拖拽跟随，跨所有复用组件支持直接手势拖拽
[修复] 修复首页底栏集成搜索按钮时的指示器拖拽中断，保留展开内容与平滑过渡
[更改] 分段控件指示器扁平化，静止内边距对齐首页底栏，并恢复内容分散排列与高度表现
[优化] 放大首页与动态顶栏指示器，外露按压绽放动画与视口边缘；动态指示器紧密跟随翻页
[修复] 修复顶栏渐进模糊接缝，还原纯净平滑的模糊转场
[优化] 共享液态玻璃渲染配置，对齐运行时渲染预算，避免模糊滤镜下的操作卡顿

### 播放器与横屏旋转
[修复] 手机端保留实际检测到的横屏朝向，全屏左右横屏不回跳
[修复] 封面分类发生变化时保持全屏重力方向监听
[修复] 调整小宽度横屏下的弹幕输入栏阈值（提升至 720dp），防止折叠屏外屏等场景下胶囊塌陷

### 直播间与弹幕互动
[优化] 桌面式横屏分栏布局右侧聊天栏宽度限制（280~400dp），分栏模式下右侧聊天列常驻
[添加] 直播发送弹幕面板新增表情入口并联动表情面板，大屏下限制最大宽度（640dp）居中展示
[优化] 直播间点赞按钮新增数值爆发缩放动效，使用标准 Filled 图标按钮统一操作体验

比较：[beta.23 整理提交…main](https://github.com/jay3-yy/BiliPai/compare/d4852346d...main)。本次仅更新源码与文档，未编译、打包或进行实机验收。

---

## v0.2.3-beta.23 (2026-09-03)

更新日志(ChangeLog)

[更改] 版本号 0.2.3-beta.23，versionCode 340
[更改] 正式包 BiliPai-0.2.3-beta.23.apk，Dev 包 BiliPai-0.2.3-beta.23-dev.apk
[更改] 基准为 beta.22 版本整理提交（fa18c6a13，versionCode 339）

### 卡片转场
[修复] 从列表进详情再返回时整卡飞回，落点对齐真实列表卡，不再停在空底板或只剩封面
[更改] 返回交接改为冻住点击时的列表卡像素，标题、间距和底部信息与静止卡片一致
[适配] 空间、相关推荐、单列、收藏、历史、稍后再看、动态及其他列表卡同步该交接
[适配] 播放量显示在封面内时，交接带上静止封面的播放量、弹幕、时长和底部黑色渐变

### 播放器
[修复] 封面页保持方向监听，全屏左右横屏均可使用
[修复] 从空间返回同一视频时重新加载播放，避免画面停在旧状态
[修复] 切换分P时清晰度可回退到可用档，避免切P后无法播放

### 界面与列表
[修复] 非液态玻璃模式下按钮、芯片和提示条使用对应原生控件
[修复] 旧系统分段标签的选中项保持可见
[修复] 动态瀑布流加载更多更稳，刷新时不再把阅读位置跳走
[修复] 历史记录删除在部分系统上不再崩溃

比较：[beta.22 整理提交…main](https://github.com/jay3-yy/BiliPai/compare/fa18c6a13...main)。本次仅更新源码与文档，未编译、打包或进行实机验收。

---

## v0.2.3-beta.22 (2026-09-02)

更新日志(ChangeLog)

[更改] 版本号保持 0.2.3-beta.22，versionCode 339
[更改] 基准为 beta.21 版本整理提交（57f6b89da，versionCode 337）

### 播放器与方向
[修复] 全屏方向按实际检测结果保留左右横屏，避免向右旋转后自动回跳
[修复] 自动连播后同步新视频身份、恢复播放画面和手势，避免回跳上一集或画面不可见
[修复] 播放结束后仍可折叠播放器，折叠状态点击“立即播放”时使用放大恢复动画 @converk
[更改] Miuix 亮度与音量手势提示改为顶部横向等级滑条，减少对播放内容的遮挡

### Miuix 与界面
[添加] 新增非液态玻璃 Miuix 展示模式，统一页面导航、设置、卡片与分段控件的形状和层级
[修复] 液态标签与历史标签恢复直接拖拽，隔离不同模式的尺寸计算并移除重复分隔线
[修复] 原生 Miuix 顶栏不再吞掉页面滚动，导航按钮保持可访问的最小触摸区域
[修复] 搜索结果、播放器设置和引导页操作按钮按当前 Miuix 模式统一文字与圆角
[添加] 可选的 Miuix 卡片深度预测返回动画及预览幅度限制，改善手势进度和指示器跟随

### 更新与设置
[更改] MD3 与 Miuix 更新弹窗分别使用对应主题的原生排版，改善长文本换行和信息层级
[更改] 更新弹窗使用右上角关闭按钮，正式版与测试版下载按钮居中排列
[修复] 更新日志独立滚动，下载操作固定显示在弹窗底部，长日志无需滑到底即可下载
[修复] MD3 设置页的日志导出项与同组设置保持一致对齐

### 动态、性能与稳定性
[添加] 动态页顶部“布局 / 发布”操作区折叠开关，折叠后保留展开入口并记住当前状态
[修复] 动态增量刷新保留阅读位置，避免列表突然跳动
[更改] 非活动页面跳过导航层截取，并仅在需要时进行页面 Chrome 捕获
[修复] 性能报告区分保留帧覆盖率与慢帧节奏，支持厂商帧标记并保留转场检查点
[修复] 插件延迟注册前先完成 Context 初始化，避免启动阶段状态不完整

比较：[beta.21 整理提交…main](https://github.com/jay3-yy/BiliPai/compare/57f6b89da...main)。本次仅更新源码与文档，未编译、打包或进行实机验收。

---

## v0.2.3-beta.21 (2026-08-31)

更新日志(ChangeLog)

[更改] 版本号保持 0.2.3-beta.21，versionCode 337
[更改] 基准为已发布的 v0.2.3-beta.20（35f001f6e，versionCode 335），合并其后变更，排除已回滚内容
[更改] 视频卡片开合转场按移动距离与尺寸变化调整时长，保留快速、标准、慢速和自定义设置
[更改] 返回转场加入轻微落位压缩；手势保持线性跟随，取消从当前姿态恢复，透明度与模糊不参与弹性变化
[修复] 统一卡片、背景、标题、正文和播放器封面的转场进度，收敛进场加载、媒体交接与返回结束时机
[更改] 转场模糊效果按会话缓存，新增分阶段帧耗时、内存报告及 60/120Hz 曲线检查工具
[修复] 液态导航指示器纵向挤压、绘制层级和宽度切换突变，减小导航图标放大与上移幅度以避免裁切
[修复] 标签宽度按长标题适配，原生标签横向滚动限制在自身区域，避免内容越界
[适配] Android 12 及更早系统的首页顶部与底部导航遮挡、文字可读性
[修复] 折叠屏外屏在关闭自动旋转时的播放器方向处理，按当前窗口区分外屏与大屏布局
[修复] 横屏播放器“更多”文字换行，悬浮评论输入栏底部手势区域保持透明
[移除] 暂时隐藏登录页的扫码确认入口，保留 TV 扫码登录

比较：[v0.2.3-beta.20…main](https://github.com/jay3-yy/BiliPai/compare/v0.2.3-beta.20...main)。本次仅更新源码与日志，未编译、打包或进行实机验收。

---

## v0.2.3-beta.20 (2026-08-31)

更新日志(ChangeLog)

[更改] 版本号 0.2.3-beta.20，versionCode 335
[更改] beta.19 已完成本地版本更新但未推送到 GitHub；本次以 beta.20 作为公开推送版本
[更改] 正式包 BiliPai-0.2.3-beta.20.apk，Dev 包 BiliPai-0.2.3-beta.20-dev.apk（versionCode 335）
[更改] 基准为远端已公开的 v0.2.3-beta.17；beta.19 未形成 GitHub 公开版本记录

### 液态玻璃与全局导航
[修复] 首页顶部、动态、搜索、视频、直播、番剧、空间及筛选栏统一液态指示器的拖拽释放逻辑，避免二次回弹
[修复] 保留点击切换时的放大、移动、缩小动效，并缩短缩放回弹时间
[修复] 首页顶部标签在分页滑动期间即时跟随，避免结束后出现延迟位置追赶

### Aicu 查询与富文本
[修复] 修复 Aicu 查询结果表情映射传递错误，恢复 B 站表情渲染
[修复] 修复液态 Dock 动画回调参数调用错误，避免设计系统编译失败

### 视频详情、评论与播放器
[更改] 视频详情内容区域按数据、互动、评论、笔记、展示和操作职责拆分，降低页面状态耦合
[修复] 评论排序、楼中楼加载、评论定位及创作团队头像跳转更加稳定
[添加] AI 摘要原生入口、视频笔记编辑与时间戳交互，支持失败重试和状态恢复
[修复] 播放器横竖屏切换、手势圆环、封面交接、自动连播身份和沉浸式侧栏布局问题
[更改] 暴露视频比例并将音频质量入口整合到播放器菜单，补齐竖屏播放器控制

### 搜索、动态、直播与分区
[修复] 搜索排序、分页、布局切换和短页继续加载逻辑，统一标签滚动行为
[修复] 动态页顶部模糊开关、UP 列表折叠及横向标题区域层级
[更改] 直播竖屏观看控制与直播聊天展示适配当前主题
[修复] 分区侧栏指示器、标签可读性、窄屏标题换行和区域信息回退

### 登录、安全与稳定性
[添加] 官方电视扫码确认及加密 BiliPai 会话转移，支持大尺寸二维码分块传输
[修复] 扫码确认绑定当前账号 Cookie，使用 Android Keystore 保护本地会话与传输密钥
[修复] 启动异常诊断、崩溃报告持久化和异常进程退出快照，限制报告范围避免保存普通历史
[更改] 图片加载迁移至 Coil 3.5.0，收紧 R8 规则并优化启动与首页封面预加载

### 工程与兼容性
[更改] AGP 升级至 9.3.2，保持现有 Compose、Kotlin 和多模块构建配置
[修复] 平板、窄屏、沉浸式播放和边缘手势下的布局安全区与交互冲突

---

## v0.2.3-beta.17 (2026-08-29)

更新日志(ChangeLog)

[更改] 版本号 0.2.3-beta.17，versionCode 327
[更改] 正式包 BiliPai-0.2.3-beta.17.apk，Dev 包 BiliPai-0.2.3-beta.17-dev.apk
[更改] 基准 v0.2.3-beta.16（5ce1ecf6c），比较区间见 GitHub Compare

### 竖屏视频与播放器
[添加] 播放设置新增“竖屏刷视频仅推荐真竖屏（Beta）”，默认关闭；开启后按实际画面方向过滤横屏推荐，关闭后保持横竖屏混合推荐
[修复] 统一设置页、Story 流与详情页竖屏入口的关闭初值，避免持久化设置读取前短暂启用过滤
[修复] 竖屏播放器的页面切换、主播放器同步、封面交接、弹幕安全区与推荐预取时机更加稳定
[修复] 竖屏合集切集保留合集上下文，合集信息、订阅状态与沉浸式样式正确延续
[修复] 画质菜单合并播放器与详情画质能力，加载阶段补齐可请求的低档画质并保持列表可滚动
[修复] 竖屏推荐在仅真竖屏模式下提前补充短列表，并支持通过 aid 查询缺少 bvid 的视频方向
[修复] 评论排序栏纳入正常布局，避免遮挡评论列表；播放器比例弹层与沉浸合集区域布局进一步简化
[修复] 平板影院模式按实际主栏宽度计算播放器视口，并补充竖屏全屏路由与折叠遮罩回归保护

### 动态与消息分享
[添加] 动态支持富文本编辑、预约切换、不感兴趣持久化、检查动态、保存图片及分享至站内消息
[添加] 动态原生音乐、收藏夹、课程与订阅直播卡片补齐展示和导航，话题流与空间动态同步接入
[添加] 站内消息分享使用原生 IM gRPC 分享卡片，并复用用户资料补全逻辑显示真实昵称与头像
[修复] 动态点赞使用乐观状态与请求闸门，避免重复点击并发、计数漂移及列表/详情状态不一致
[修复] 动态预约解析服务端按钮状态、人数和跳转信息，列表与空间页共享交互结果
[修复] 动态瀑布流图片预览边界不再写回 Compose 状态，降低滚动重组与布局抖动
[修复] 收藏夹卡片保留服务端副标题与徽标，发布器编辑时保留已有图片、标题和话题
[测试] 补充动态解析、管理操作、保存分享、消息 gRPC、瀑布流稳定性及 Compose UI 回归测试

### 界面、直播与空间
[修复] 顶部标签与底部导航等待持久化玻璃设置后再渲染，避免启动首帧出现临时样式闪切
[修复] 直播聊天气泡改用语义主题色，在明暗主题下提升用户名、正文与回复文字对比度
[修复] 空间页从关系接口同步真实关注状态，关注操作期间阻止重复请求并在失败时可靠回滚
[修复] 空间加载骨架使用与关闭原生液态玻璃一致的安全初始样式

格式规范见 [docs/wiki/CHANGELOG_GUIDE.md](docs/wiki/CHANGELOG_GUIDE.md)。比较：[`5ce1ecf6c...main`](https://github.com/jay3-yy/BiliPai/compare/5ce1ecf6c...main)。

---

## v0.2.3-beta.16 (2026-08-28)

更新日志(ChangeLog)

[更改] 版本号 0.2.3-beta.16，versionCode 326
[更改] 正式包 BiliPai-0.2.3-beta.16.apk，Dev 包 BiliPai-0.2.3-beta.16-dev.apk
[更改] 基准 v0.2.3-beta.15（54ea52e74），比较区间见 GitHub Compare

### 视频、评论与播放器
[修复] 视频详情返回列表时的封面、内容与操作栏交接更连贯，首帧前形变稳定，避免闪烁或空白
[修复] 游客评论优先使用可访问的主评论列表接口，改善未登录场景的评论加载
[修复] 关闭自动旋转后播放器不再自动切换横屏

### 首页、话题与交互
[修复] 非液态玻璃模式恢复原生顶部标签布局；窄屏标签完整可见，宽屏热门标签居中显示
[修复] 话题切换、排序栏背景与评论编辑器层级保持独立，避免液态玻璃背景串扰
[添加] 动态话题支持原生入口与独立标签展示

### 设置
[添加] 设置中新增作者赞助入口，并与现有分类及说明样式对齐

格式规范见 [docs/wiki/CHANGELOG_GUIDE.md](docs/wiki/CHANGELOG_GUIDE.md)。比较：[`54ea52e74...main`](https://github.com/jay3-yy/BiliPai/compare/54ea52e74...main)。

---

## v0.2.3-beta.15 (2026-08-28)

更新日志(ChangeLog)

[更改] 版本号 0.2.3-beta.15，versionCode 324
[更改] 正式包 BiliPai-0.2.3-beta.15.apk，Dev 包 BiliPai-0.2.3-beta.15-dev.apk
[更改] 基准 v0.2.3-beta.14（91f834dfe），比较区间见 GitHub Compare

### 动态与富文本
[修复] 动态详情与简介的完整 Opus 正文、表情短码、公式节点和图片在刷新回填后完整保留，不再重复渲染或丢失
[修复] 动态富文本中的 @ 提及与链接元数据在截断正文、全文与详情合并时保留，高亮和跳转可点击
[添加] 发布的直播标识动画，折叠动态用户列表时顶部 Dock 显示与会话过渡更平滑

### 首页、导航与空间
[更改] 首页顶部与底部 Dock、空间媒体标签等统一使用共享液态指示器和弹性拖动，跨屏动效一致
[修复] 底部 Dock 的按下光晕、选中项缩放、圆角裁剪与指示器同步，手势归位后图标比例归一
[修复] 空间文章查看数在不可用时隐藏，标题表情符号与正文字体样式保留，直播链接跳转原生直播间
[更改] 相关推荐与横向视频卡片主题表面统一，返回时封面挂载无缝衔接，收藏默认索引与加载策略优化

### 设置、图标与稳定性
[修复] Material 3 设置搜索与播放设置补齐缺失的语义图标，原生对话框图标对齐
[修复] 卡片落地标题裁剪、详情标签栏/横向卡片过渡及滚动几何相关的多处稳定性问题
[修复] 补充紧凑标签宽度回归等 UI 结构与策略测试

格式规范见 [docs/wiki/CHANGELOG_GUIDE.md](docs/wiki/CHANGELOG_GUIDE.md)。比较：[`91f834dfe...main`](https://github.com/jay3-yy/BiliPai/compare/91f834dfe...main)。

---

## v0.2.3-beta.13 (2026-08-26)
...
### 首页、导航与液态玻璃
[添加] 首页推荐流升级为瀑布流布局并重做 Hero 轮播交互，卡片标题、作者、播放与发布时间信息保持完整
[更改] 首页 Hero 调整到顶部标签下方，手机、平板和侧栏场景统一内容间距、首列占位与滑动阈值
[添加] 顶部渐进模糊支持强度调节，并复用到首页、历史、音乐及其他浮动顶栏
[更改] 顶部标签、直播分类、番剧筛选、历史筛选、空间媒体标签与底部 Dock 统一使用共享液态指示器和弹性拖动
[更改] 全局栏位模糊开关统一控制顶部、底部、搜索、评论与操作 Dock，关闭玻璃时按 Miuix/Material 主题回退原生控件
[修复] 可滚动液态 Dock 的左右圆角裁剪、窄槽闪退、初始回退崩溃、选中项边距、标签省略与拖动可见性
[修复] 视频详情“简介 / 评论”液态指示器恢复左右拖动，并与内容分页位置实时同步
[修复] 历史页标题折叠后保持搜索与筛选悬浮，返回顶部恢复标题；搜索框正确采样玻璃背景并取消主题色染色
[修复] 搜索进入/退出与底栏动画双向衔接，避免返回首页白屏、状态跳变或 Dock 归位错位

### 搜索、番剧、直播与动态
[添加] 搜索补齐文章、图文等结果类型和筛选入口，并加入排序卡片过渡
[添加] 番剧中心支持范围筛选、日期时间线、分区搜索与国创索引，改善未登录及受限内容的播放提示
[添加] 直播间支持只读投票面板，直播首页和二级分类统一主题分流与玻璃 Dock
[修复] 动态 Opus 正文、表情短码和详情元数据完整保留，充电视频内容与富文本展示恢复正常
[更改] 动态详情评论、发布器和操作控件拆分为独立玻璃层，修正循环 Backdrop、浮动布局和指示器跟随
[添加] 插件命中提示开关与直播用户标识动画，皮肤导入补齐更多语义表面映射

### 播放器、媒体与内容卡片
[更改] 播放器画质、音质、分集、章节、合集、比例和播放顺序菜单统一为 Miuix/Material 自适应弹层与列表组件
[添加] 播放器支持新的比例选择、合集条目、分集选择和播放顺序面板，并保留键盘、横屏与窄屏操作
[修复] 弹幕分段窗口随进度滚动加载，收藏播放状态可在后台同步，讨论区播放器和主题交互保持一致
[修复] 分P切换按 API 要求使用同一详情快照的 bvid 与目标 cid 请求，并同步详情页内部播放身份，避免旧 cid 回写后跳回 P1
[更改] 竖屏详情上滑收起播放器时，在内容顶部加入随折叠进度增强的渐变阴影，缓和播放器与简介栏的切换边界
[更改] 首页、历史、稍后再看、分区、个人空间、收藏和相关推荐统一完整卡片元数据、骨架布局及 UP 标识开关
[修复] 相关推荐样式对齐 PiliPlus，详情标签、操作反馈、评论栏与顶部玻璃的边距和层级保持稳定
[修复] 视频选集卡片为长短标题统一预留两行，避免同一行卡片高度不一致
[修复] 分P切换先提交目标 cid 与界面状态再替换播放源，避免播放器同步回调将选集恢复到 P1

### 设置、图标与稳定性
[更改] 设置页导航、条目、对话框、菜单、图标、开关和选择控件进一步统一 Miuix/Material 原生渲染
[添加] Material Symbols 资源与语义图标映射，设置搜索、分类入口和播放器菜单图标更加一致
[更改] 壁纸设备标签改用自适应原生分段控件，皮肤目录、设置搜索和窗口菜单共享统一组件
[修复] Material 3 主题使用原生列表组件时，“主题色容器”图标样式可正确应用到普通条目与开关条目
[修复] 圆形启动图标目标资源、系统通知页面、主题详情卡片、平板侧栏和底部 Pager 状态连续性
[修复] 多处 Compose 权重、滚动几何、重组状态、返回导航和玻璃采样边界问题，补充对应结构与策略测试

格式规范见 [docs/wiki/CHANGELOG_GUIDE.md](docs/wiki/CHANGELOG_GUIDE.md)。比较：[`2df4685...main`](https://github.com/jay3-yy/BiliPai/compare/2df4685...main)。
